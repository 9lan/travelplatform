...
async function main() {
  ...
  app.get('/health', async (_, res) => {
    try {
      ...
      res.status(503).json({ status: 'error', service: 'notification-service', error: 'Database connection failed' });
    }
  });
  ...
  app.listen(port, () => {
    console.log(`Notification Service ready at http://localhost:${port}/graphql`);
