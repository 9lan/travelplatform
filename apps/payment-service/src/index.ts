...
async function main() {
  ...
  app.get('/health', async (_, res) => {
    try {
      ...
      res.status(503).json({ status: 'error', service: 'payment-service', error: 'Database connection failed' });
    }
  });
  ...
  app.listen(port, () => {
    console.log(`Payment Service ready at http://localhost:${port}/graphql`);
