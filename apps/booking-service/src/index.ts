...
async function main() {
  ...
  app.get('/health', async (_, res) => {
    try {
      ...
      res.status(503).json({ status: 'error', service: 'booking-service', error: 'Database connection failed' });
    }
  });
  ...
  app.listen(port, () => {
    console.log(`Booking Service ready at http://localhost:${port}/graphql`);
